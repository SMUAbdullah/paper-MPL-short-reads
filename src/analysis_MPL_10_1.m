...
% % if(FLAG_UserProvidedRefSeq == true) % comment after testing
...
%             refSeq = seqNT_All{1};
        ...
        %         disp(num2str(q11_sp)); % remove after testing
        %         disp('gap'); % remove after testing
        if(FLAG_Epi == true)
        ...
        if(FLAG_stratonovich == false)
            if(FLAG_linearInt == false)
                if(tp == 1)
                    ...
                    ICM_pt(:,:,tp)=tempintCovMtx;%%%%%%%%%%%%%
                    %                 whos q_sp
                    %                 whos intCovMtx
                    %                 pause
                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    %                     disp(['t: ' num2str(tp)]); % remove after testing
                    %                     disp([num2str(tempintCovMtx(1,1)) ' ' num2str(tempintCovMtx(2,2))]); % remove after testing
                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            elseif(FLAG_linearInt == true) % linear interpolation
                ...
                if(tp == 1)
                    ...
                    ICM_pt(:,:,tp)=thisCovMtx;%%%%%%%%%%%%%
                    intCovMtx = intCovMtx + thisCovMtx*timeStep(tp-1);
                    
                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    %                     disp(['t: ' num2str(tp)]); % remove after testing
                    %                     disp([num2str(thisCovMtx(1,1)) ' ' num2str(thisCovMtx(2,2))]); % remove after testing
                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    
                    %%%%%%%%%%%%%%%%%%
                    %                     disp(num2str(full(thisCovMtx*timeStep(tp-1)))); % remove after testing
                    %                     disp('gap'); % remove after testing
                    %%%%%%%%%%%%%%%%%%
...
%                         last_q1111_sp = this_q1111_sp;
        ...
        elseif(FLAG_stratonovich == true)
            ...
            if(tp == 1) % tell Saqib bhai
                ...
                ICM_pt(:,:,tp)=lastCovMtx;%%%%%%%%%%%%%
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                %                     disp(['t: ' num2str(tp)]); % remove after testing
                %                 disp([num2str(lastCovMtx(1,1)) ' ' num2str(lastCovMtx(2,2))]); % remove after testing
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                ...
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                %                     disp(['t: ' num2str(tp)]); % remove after testing
                %                 disp([num2str(thisCovMtxMidPoint(1,1)) ' ' num2str(thisCovMtxMidPoint(2,2))]); % remove after testing
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     fileNameAllTrajsR = ['AllTrajsWithTimeInfo_filt_thresh_' thisFileNameHUFiles(1:indOfDash(2)-1) '.txt'];
    ...
    if(FLAG_Epi == false)
        fileNameTestWorkspace = ['TestWorkspace_filt_thresh_' thisFileNameHUFiles(1:indOfDash(2)-1) '_' convention '_gamma' num2str(priorConstSC) '.mat'];
    elseif(FLAG_Epi == true)
        fileNameTestWorkspace = ['TestWorkspace_filt_thresh_' thisFileNameHUFiles(1:indOfDash(2)-1) '_' convention '_gamma' num2str(priorConstSC) '_' num2str(priorConstEpi) '.mat'];
    end
